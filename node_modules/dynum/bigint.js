var bi = (function() {
    ...
    BigInt.prototype.isGreater = function(a) {
        // TODO: make this cell based rather than bitwise
    ...
    BigInt.prototype.divMod = function(obj) {
        ...
        var resultCellsCount = this.buffer.cellCount() - obj.buffer.cellCount() + 1;
        var resultCells = new Uint16Array(resultCellsCount);
        ...
        for (var i = obj.buffer.cellCount() - 1; i >= 0; i--) {
            ...
            var thisCell = this.buffer.getCell(i + resultCellsCount - 1);
        ...
        var useIndex = 0;
        if (!isTooBig) {
            // we can do a typical division
            useIndex = resultCells.length - 1;
        } else {
            // we can't do a typical division :'(
            if (resultCells.length == 1) throw new Error('This is impossible');
            mostSig = (mostSig * 0x10000) + this.buffer.getCell(this.buffer.cellCount() - 2);
            useIndex = resultCells.length - 2;
        }
        
        // lower and upper bound for this cell
        // to be safe, we will test the high value
        // first and then if that was too high
        // we'll settle for the low value
        ...
        resultCells[useIndex] = quotientMax;
        var result = new BigInt(new BitBuffer(resultCells), true);
        var remaining = this.sub(result.mul(obj));
        
        if (!remaining.sign && remaining.bitCount() > 0) {
            resultCells[useIndex] = quotientMin;
            result = new BigInt(new BitBuffer(resultCells), true);
            remaining = this.sub(result.mul(obj));
        }
        ...
        // binary search
        var startDig = this.bitCount() - obj.bitCount();
        for (var digit = startDig; digit >= 0; digit--) {
