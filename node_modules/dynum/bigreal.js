...
var br = (function() {
    ...
    BigReal.withDouble = function(fn) {
        BigReal.withPrecision(BigReal.precision * 2, fn);
    }
    
    BigReal.fromFloat = function(theFloat) {
        if (theFloat < 0) return BigReal.fromFloat(-theFloat).neg();
    ...
    BigReal.epsilon = function() {
        return BigReal.fromFloat(1).shiftRight(BigReal.precision - 2);
    }
    ...
    BigReal.prototype.toFloat = function() {
        if (!this.number.sign) return -this.neg().toFloat();
    ...
    BigReal.prototype.isEqual = function(num) {
        if (this.number.bitCount() == 0 && num.number.bitCount() == 0) return true;
    ...
    BigReal.prototype.sqrt = function(noCheck) {
        // use the Babylonian method to compute an accurate root
        if (this.number.bitCount() == 0) return BigReal.zero;
        var one = BigReal.fromFloat(1);
        if (!this.isGreater(one) && !this.isEqual(one) && !noCheck) {
            return one.div(one.div(this).sqrt(true));
        }
    ...
    BigReal.prototype.cbrt = function() {
        // use newton's method to compute an accurate principle root
        if (this.number.bitCount() == 0) return BigReal.zero;
        if (!this.number.sign) return this.neg().cbrt().neg();
        var guess = new BigReal(this.number, Math.round(this.exp / 3));
        var difference = null;
        var epsilon = BigReal.epsilon();
        var two = BigReal.fromFloat(2);
        var three = BigReal.fromFloat(3);
        while (!difference || difference.isGreater(epsilon)) {
            // get the current error
            var guessSquare = guess.mul(guess);
            var tmpDifference = guessSquare.sub(this).abs();
            ...
            // generate the next guess
            var addTerm = this.div(guessSquare);
            guess = guess.mul(two).add(addTerm).div(three);
        }
        return guess;
    }
    ...
    BigReal.prototype.sin = function() {
        return this.circlePeriod().periodSin();
    }
    
    BigReal.prototype.cos = function() {
        return this.circlePeriod().periodCos();
    }
    
    BigReal.prototype.tan = function() {
        return this.circlePeriod().periodTan();
    }
    
    BigReal.prototype.periodSin = function() {
        var quarter = BigReal.fromFloat(1/4);
        var quadrant = Math.round(this.div(quarter).wholeFrac().whole.toFloat());
        if (quadrant == 0) {
            return this.firstQuadVector()[1];
        } else if (quadrant == 1) {
            return this.sub(quarter).firstQuadVector()[0];
        } else if (quadrant == 2) {
            return this.sub(BigReal.fromFloat(1/2)).firstQuadVector()[1].neg();
        } else {
            return this.sub(BigReal.fromFloat(3/4)).firstQuadVector()[0].neg();
        }
    }
    
    BigReal.prototype.periodCos = function() {
        var quarter = BigReal.fromFloat(1/4);
        var quadrant = Math.round(this.div(quarter).wholeFrac().whole.toFloat());
        if (quadrant == 0) {
            return this.firstQuadVector()[0];
        } else if (quadrant == 1) {
            return this.sub(quarter).firstQuadVector()[1].neg();
        } else if (quadrant == 2) {
            return this.sub(BigReal.fromFloat(1/2)).firstQuadVector()[0].neg();
        } else {
            return this.sub(BigReal.fromFloat(3/4)).firstQuadVector()[1];
        }
    }
    
    BigReal.prototype.periodTan = function() {
        var quarter = BigReal.fromFloat(1/4);
        var quadrant = Math.round(this.div(quarter).wholeFrac().whole.toFloat());
        var period = this;
        if (quadrant == 1) period = this.sub(quarter);
        if (quadrant == 2) period = this.sub(BigReal.fromFloat(1/2));
        if (quadrant == 3) period = this.sub(BigReal.fromFloat(3/4));
    ...
    BigReal.prototype.arcsin = function() {
        if (!this.number.sign) return this.neg().arcsin().neg();
        
        // if the angle is more than roughly sqrt(2)/2, then
        // take the complement and use that.
        if (this.isGreater(BigReal.fromFloat(6/8))) {
            // calculate the complement
            var cosine = BigReal.fromFloat(1).sub(this.mul(this)).sqrt();
            var complement = cosine.arcsin();
            var radCompl = BigReal.fromString(BigReal.piString).div(BigReal.fromFloat(2));
            return radCompl.sub(complement);
        }
        
        // here we go; mclauren series it upppp
        var result = BigReal.zero;
        var two = BigReal.fromFloat(2);
        var pochammer = BigReal.fromFloat(1/2);
        var one = BigReal.fromFloat(1);
        var coefficient = one;
        var epsilon = BigReal.epsilon();
        
        var polyPower = this;
        var thisSquared = this.mul(this);
        
        for (var term = 0; term < 1000000; term++) {
            var divisor = BigReal.fromFloat(term * 2 + 1);
            var useCoeff = coefficient.div(divisor);
            var last = result;
            var addTerm = useCoeff.mul(polyPower);
            result = result.add(addTerm);
            if (!addTerm.isGreater(epsilon)) {
                break;
            }
            
            // step
            coefficient = coefficient.mul(pochammer);
            coefficient = coefficient.div(BigReal.fromFloat(term + 1));
            pochammer = pochammer.add(one);
            polyPower = polyPower.mul(thisSquared);
        }
